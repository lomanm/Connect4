...
public class C4Console implements C4View {
	...
	public void printInitialBoard(C4Model model) {
		printBoard(model, new int[2]);		// the 2nd parameter is a formality in console view
	}
	
	public void printBoard(C4Model model, int[] slotPlayed){	// 2nd parameter is not used in console view
		...
		for (int row=5; row>=0; row--){					// print from top (row 6) to bottom (row 1)
			...
			for (int column=0; column<=6; column++){		// print from left (col 1) to right (col 7)
				System.out.print(model.getBoard().getSlot(row,column) + (column < 6 ? "|" : ""));
			}
			System.out.println("|");
		}
		System.out.println("|-------------|");
	...
	public void printGameOver(int winner, String winText, C4Model model){
		switch (winner) {
			...
			case 3:
				...
				break;
			default:
				System.out.println("Game over, you " + winText + "!");	// prints "you win" or "you lose"
		}
	}
	...
	public int getHumanChoice(C4Model model) {
		...
		if (columnChosen<1 || columnChosen>7){
			...
			return 0;
		} else if (!model.getBoard().getSlot(5,columnChosen-1).equals(" ")){
